    ...
    return Column(
        children: [
                  ...
                  child: TextField(
                    ...
                    onChanged: (value) {
                      ...
                      if ( (value.contains('{') || value.contains('^2') || value.contains('[')
                        ...
                        // QuizController.to.question_tex.value = r'' + QuizController.to.question.value.replaceAll('{', r'\({').replaceAll('}', r'}\)').replaceAll('/', r'\over').replaceAll('\n', r'$$ $$')
                        //     .replaceAll('[', '□').replaceAll(']', '').replaceAll('times', r'\times').replaceAll('div', '÷').replaceAll('^2', r'\({^2}\)')
                        //     .replaceAll('L', r'\({L}\)').replaceAll('km', r'\({km}\)').replaceAll('m', r'\({m}\)').replaceAll('cm', r'\({cm}\)').replaceAll('mm', r'\({mm}\)');
                      }
                      else {
                        QuizController.to.question.value = value;
                      }
          ...
          Obx(() => Visibility(
            ...
            child: Stack(
                children: [
                  ...
                  ClipRRect(
                    borderRadius: BorderRadius.circular(5.0),
                    child: Image.network(QuizController.to.question_image_url.value, fit: BoxFit.cover, width: 300,),
                  ),
                  IconButton(
          ...
          Column(
            children: [
              Row(
                mainAxisAlignment: QuizController.to.question_type.value == '단답형' ? MainAxisAlignment.center : MainAxisAlignment.spaceAround,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  /// 1번
                        ...
                        Row(
                          children: [
                            SizedBox(
                              width: 680,
                              child: TextField(
                                controller: TextEditingController(text: QuizController.to.answer.value),
                                textAlignVertical: TextAlignVertical.center,
                                onChanged: (value) {
